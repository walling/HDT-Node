...
HdtDocumentPrototype.suggestions = function (prefix, options, callback, self) {
  if (typeof callback !== 'function') self = callback, callback = options, options = {};
  if (typeof callback !== 'function') return;
  if (this.closed) return callback.call(self || this,
                          new Error('The HDT document cannot be read because it is closed'));
  var limit     = options && options.limit ? Math.max(0, parseInt(options.limit, 10)) : 0,
      component =
        (options && options.component ===   'subject') ? 1 :
        (options && options.component === 'predicate') ? 2 :
                                                         3; // 'object' is default
  this._suggestions(prefix, component, limit, callback, self);
};
